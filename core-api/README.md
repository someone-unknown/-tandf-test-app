...
# Notes
* All dates are stored in ISO format and by default for simplicity are considered to be in UTC timezone. The timezone handling is out of scope of this task.
* For simplicity the backend will not handle situation when start time and end time are different days as I don't know if it's possible at all or how it should be handled and how the time should be transferred for the next days.
